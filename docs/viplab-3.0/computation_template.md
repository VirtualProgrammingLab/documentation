...
|configuration --running.intermediateFilesPattern | List of Strings (Update-Patterns) to specify when new intermediate file was written |opt | regex-expression in stdout which file is ready to be transferred | for **Container**| Is this implemented?|
...
|Key [--Subkey] | Type (a default is marked by _italics_)|Opt / Must |Description |Comment | AS |
|---------------|----------------------------------------|-----------|------------|--------|----|
|viewer | one or more (list) of ["Image", "ParaView", "ViPLabGraphics", "CSV"] | opt | When given, specific file extension, like ".vtu" are interpreted by the frontend for displaying results. Otherwise files are only downloadable. | | |
|csv | struct | opt | Use this object to define connected csv-files, if there are intermediate results | | See [csv json object](#csv-json-object) for details on contained objects |
|vtk | struct | opt | Use this object to define connected vtk-files, if there are intermediate results | | See [vtk json object](#vtk-json-object) for details on contained objects |
