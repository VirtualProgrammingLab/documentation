...
|configuration --running.intermediateFilesPattern | [ REGEX, REGEX, ... ] | opt | regex-expression in stdout which file is ready to be transferred as intermediate result. The Regex has to return the complet filename as matching group 1 | for **Container**|
...
|basename | string | opt | Basename defines related files: Path of file begins with basename | Files transmitted in intermediate results are also marked as being related, if the match the basename |
...
|plots | [ {...}, {...}, ... ] | opt | Define datasets: Provide y-axis labels for y-key given in csv |For each dataset one diagram is rendered in the result |
|plots --key | string | opt | key for y-axis given in csv header | |
|plots --label | string | opt | y-axis label for diagram | |
|plots --factor | string | opt | Multiply y-values with this factor | |
